...
ROOTGLIBS     = $(shell root-config --glibs) -lGenVector -lFoam -lMinuit -lTMVA -lMLP -lXMLIO -lTreePlayer -lMathMore -lRooFit -lRooFitCore -lRooStats
...
all: cfgMan $(LIB)$(SONAME) $(LIBS) $(BINS)
...
$(BIN)%$(BINSuf): $(PRG)%$(PRGSuf) $(HDRS) $(LIB)$(SONAME)
	$(CPP) $(CPPFLAGS) $(GLIBS) -L$(LIB) -lPGunAnalysis -o $@ $<
