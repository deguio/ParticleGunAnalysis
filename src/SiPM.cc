...
HcalSiPM::HcalSiPM(int nCells, double tau, TRandom3* rnd) :
...
unsigned int HcalSiPM::addCrossTalkCells(TRandom3* engine,
  ...
  double HcalSiPM::hitCells(TRandom3* engine, unsigned int pes, double tempDiff,
    double photonTime) {
      ...
      for (unsigned int pe(0); pe < pes; ++pe) {
        pixel = (int)(theCellCount*engine->Rndm());
        hit = (theSiPM[pixel] < 0.) ? 1.0 : (cellCharge(photonTime - theSiPM[pixel]));
        sum += std::floor((hit*(1 + (tempDiff*theTempDep)))); //FDG doesn't discharge if not fully charged
        theSiPM[pixel] = photonTime;

        //std::cout << "hit: " << hit << " sum: " << sum << std::endl;
      }
    ...
    double HcalSiPM::cellCharge(double deltaTime) const {
      ...
      return (result > 0.99) ? 1.0 : result; //consider fully charged above 0.99
    }
